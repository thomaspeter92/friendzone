...
<div class="container ">
  <div class="row">
    <form class="col-md-6 mx-auto" action="<?= PROJECT_ROOT_PATH ?>/make-post" method="POST">
        ...
        foreach ($posts as $post) : ?>
          <div class="card  text-bg-light mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
              <div>
                <h6 class="mb-0"><a href="<?= PROJECT_ROOT_PATH ?>/profile?user_id=<?= $post['author_id'] ?>"><?= $post['first_name'] . ' ' . $post['last_name'] ?></a></h6>
                <p class="mb-0 text-secondary"><small><?= $post['created_at'] ?></small></p>
              </div>
              <?php
              if ($_SESSION["user_id"] == $post['author_id']) { ?>
                <a href="<?= PROJECT_ROOT_PATH ?>/delete-post?post_id=<?= $post['id'] ?>" type="button" class="btn-close" aria-label="Delete"></a>
              <?php
              }
              ?>
            </div>
